...
class BullAnalyst:
    ...
    async def analyze(self, trigger_time: str = None, symbol: str = "000001") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"🐂 开始看涨分析: {symbol} 在 {trade_date}")
            ...
            # 生成看涨分析
            bull_analysis = await self._generate_bull_analysis(
    ...
    async def _generate_bull_analysis(self, symbol: str, trade_date: str, 
                                    financial_report: str, hot_money_report: str, 
                                    price_report: str, news_report: str, stock_analysis_report: str) -> str:
        """生成看涨分析"""
        try:
            # 构建分析提示词
            prompt = f"""你是一位专业的看涨分析师，负责为股票 {symbol} 的投资建立强有力的论证。
...
请基于以上五个数据源的信息，生成一份专业的看涨分析报告，要求：

1. **结构清晰**：按照上述5个分析重点组织内容
2. **数据支撑**：每个观点都要有具体的数据和事实支撑
3. **逻辑严密**：论证过程要逻辑清晰，结论要有说服力
4. **客观专业**：保持专业分析师的客观立场，避免过度乐观
5. **实用性强**：提供具体的投资建议和风险提示
...
"""
            ...
            messages = [
                {
                    "role": "system", 
                    "content": "你是一位资深的看涨分析师，专长于股票投资分析和市场研究。请基于实际数据生成专业的看涨分析报告。"
                },
            ...
            if response and response.content:
                ...
                logger.error(f"LLM看涨分析未返回内容")
                return f"{symbol} 看涨分析生成失败"
                
        except Exception as e:
            logger.error(f"生成看涨分析失败: {e}")
            return f"{symbol} 看涨分析失败: {str(e)}"


if __name__ == "__main__":
    # 测试看涨分析师
    async def test_bull_analyst():
        analyst = BullAnalyst()
        ...
        print(f"\n看涨分析报告:")
        print("-" * 50)
        print(result['bull_analysis'])
    
    asyncio.run(test_bull_analyst())
